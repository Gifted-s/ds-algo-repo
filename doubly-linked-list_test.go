...
func TestDoublyLinkList(t *testing.T) {
	...
	t.Run("Add Method", func(t *testing.T) {
		...
		t.Run("Expect head and tail to point to thesame node if its the first element added to the linked list", func(t *testing.T) {
			new_list := DoublyLinkedList{}
			new_list.Add(4)
			
			if new_list.head.data != 4{
				t.Fail()
			}
			if new_list.tail.data != 4{
				t.Fail()
			}
		})
	...
	t.Run("RemoveFirst Method", func(t *testing.T) {
	...
	t.Run("AddFursrt Method", func(t *testing.T) {

		t.Run("Expect AddToHead to return value at head", func(t *testing.T) {
			new_list := DoublyLinkedList{}
			if new_list.AddFirst(2) != 2 {
				t.Fail()
			}
			if new_list.AddFirst(3) != 3 {
				t.Fail()
			}
		})
